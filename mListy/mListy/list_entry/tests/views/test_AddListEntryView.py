...
class AddListEntryViewTests(BaseListEntryTestClass):
    ...
    def setUp(self):
        super().setUp()
        self.VALID_LIST_ENTRY_DATA_2['list'] = self.user_list.id
        self.VALID_LIST_ENTRY_DATA_2['movie'] = self.movie
    ...
    def test_add_entry_to_list_with_valid_credentials(self):
        self.post_response_for_create_list_entry(self.movie, self.VALID_LIST_ENTRY_DATA_2)
    ...
    def test_status_code_after_valid_add_entry_to_list_operation__expect_302(self):
        response = self.post_response_for_create_list_entry(self.movie, self.VALID_LIST_ENTRY_DATA_2)
